...
export class KnitNode {
  ...
  constructor(
    type: KnitNodeType = KnitNodeType.KNIT,
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (this.previous_node) {
        ...
        let traversal = [
          ...
          { dir: KnitEdgeDirection.ROW, in: backdir },
        ];
...
export class KnitGraph<N extends KnitNode = KnitNode, E extends KnitEdge = KnitEdge> {
  ...
  neighbours(node: N): N[] {
    return this.edgesof(node)
      .map((edge) => Object.values(this.nodes).find((n) => node.id != n.id && (n.id == edge.from || n.id == edge.to)))
      .filter((node) => node) as N[];
  }
