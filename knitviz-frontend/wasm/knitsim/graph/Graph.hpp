...
namespace knitsim {
  class KnitGraphC {
    ...
    */
    ...
    std::map<uint32_t, std::vector<Eigen::Vector3f>> knitpaths;
    GraphConfig config;
   ...
   public:
    ...
    Node getNode(uint32_t id) const { return *node_map.at(id); }
    ...
    std::vector<Edge> edgesOf(Node &node, std::optional<KnitEdgeDirectionC> direction = std::nullopt) const {
      ...
      for (auto edge : edges) {
        if ((edge.from == node.id || edge.to == node.id) &&
            (direction == std::nullopt || edge.direction == direction)) {
    ...
    std::vector<Node>
    incomingNodes(const Node &node, std::optional<KnitEdgeDirectionC> direction = std::nullopt) const {
      std::vector<Node> result;
    ...
    std::vector<TraversalResult>
    traverseGraph(Node &start_node, std::vector<TraversalInstruction> &instructions) const {
    ...
    RowResult row(Node &start_node) {
      ...
      auto start_iter =
          std::find_if(res.nodes.begin(), res.nodes.end(), [](const Node &node) { return node.start_of_row == true; });
